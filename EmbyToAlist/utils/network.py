...
async def stream_handler(
    ...
    try:
        file_headers = await cache_system.storage.get_file_headers(
            request_info.file_info
        )
        if file_headers:
            ...
            if (
                file_headers.content_disposition
                and "Content-Disposition" not in response_headers
            ):
                response_headers["Content-Disposition"] = (
                    file_headers.content_disposition
                )
            logger.debug(
                f"Added cached headers to response: ETag={file_headers.etag}, Last-Modified={file_headers.last_modified}"
            )
        ...
        try:
            ...
            if response_range is None:
                raise fastapi.HTTPException(
                    status_code=500, detail="Response range is not set"
                )
            ...
            if (
                cache is None
                and request_info.cache_range_status != CacheRangeStatus.NOT_CACHED
            ):
            ...
            async for chunk in _cache_stream:
                ...
                if (
                    SMART_CACHE_FALLBACK
                    and (time.time() - cache_start_time) > SMART_CACHE_TIMEOUT
                ):
                    logger.warning(
                        f"Cache streaming timeout ({SMART_CACHE_TIMEOUT}s) reached. Sent {bytes_sent} bytes.强制切换为302/代理"
                    )
                    return
            ...
            logger.debug(
                f"Media client is high compatibility: {request_info.is_HIGH_COMPAT_MEDIA_CLIENTS}"
            )
            if request_info.is_HIGH_COMPAT_MEDIA_CLIENTS:
                logger.debug(
                    "High compatibility client finished cache segment, ending stream"
                )
                if remaining_total > 0:
                    logger.warning(
                        "High compatibility client expected to finish within cache, but remaining data detected"
                    )
                return

            if FORCE_CLIENT_RECONNECT:
                logger.info(
                    "FORCE_CLIENT_RECONNECT is enabled, ending stream to force client reconnect"
                )
                return
            ...
            logger.info(
                f"Falling back to reverse proxy from {reverse_start} to {reverse_end}"
            )
            ...
            async for chunk in reverse_proxy(request_info, reverse_start, reverse_end):
                ...
                if remaining_total < 0:
                    ...
                    logger.warning(
                        "Reverse proxy provided more data than expected, truncating output"
                    )
                    break
                yield chunk
                if remaining_total == 0:
    ...
    return fastapi.responses.StreamingResponse(
        merged_stream(), headers=response_headers, status_code=status_code
    )


async def reverse_proxy(
    request_info: RequestInfo, start: int, end: int, chunk_size: int = 1024 * 1024
    ...
    if request_info.raw_link_manager is None:
        raise fastapi.HTTPException(
            status_code=500, detail="Raw link manager is not initialized"
        )
    ...
    try:
        async with client.stream(
            "GET", raw_url, headers=headers, timeout=httpx.Timeout(20, connect=30.0)
        ) as response:
...
async def pure_reverse_proxy(
    request_info: RequestInfo, start_byte: int, end_byte: Optional[int], file_size: int
) -> fastapi.responses.StreamingResponse:
    """
    ...
    return fastapi.responses.StreamingResponse(
        stream(), headers=response_headers, status_code=206
    )
